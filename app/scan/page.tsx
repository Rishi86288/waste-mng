  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      img.onload = async () => {
        ...
        canvas.width = 640;
        canvas.height = img.height * (640 / img.width);
        ...
        try {
          ...
          if (data.success) {
            ...
            setErrorMsg(data.error || data.message || "Detection failed.");
          }
        } catch (e: any) {
  ...
  return (
      ...
      <div className="flex flex-col items-center py-12 px-4 w-full bg-gray-50 min-h-[80vh]">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
          <h1 className="text-2xl font-bold text-gray-900 text-center">AI Waste Scanner</h1>
          <p className="text-gray-500 text-sm text-center mt-1 mb-6">Upload to identify waste & earn points.</p>

          <div className="relative w-full bg-gray-900 rounded-xl overflow-hidden shadow-inner p-6 flex flex-col items-center justify-center min-h-/[250px] mb-6">
            {imagePreview ? (
              <img src={imagePreview} alt="Preview" className="w-full h-48 object-contain rounded-lg mb-4" />
            ) : (
              <div className="text-6xl mb-4 opacity-50">📷</div>
            )}
            <label className="bg-green-600 text-white px-6 py-2.5 rounded-lg cursor-pointer hover:bg-green-700 text-sm font-semibold transition-all">
              {imagePreview ? "Retake Photo" : "Upload Photo"}
              <input type="file" accept="image/*" className="hidden" onChange={handleFileUpload} />
            </label>
          </div>

          {errorMsg && <div className="p-4 bg-red-50 text-red-700 rounded-md text-sm mb-4">{errorMsg}</div>}
          ...
          {prediction && (
            <div className="p-5 bg-green-50 border border-green-200 rounded-xl">
              ...
              <p className="text-sm text-gray-700 font-medium">💡 {prediction.disposalInstruction}</p>
            </div>
          )}

          {isScanning && !prediction && !errorMsg && (
            <div className="flex flex-col items-center justify-center py-6">
              <div className="w-8 h-8 border-4 border-gray-200 border-t-green-600 rounded-full animate-spin"></div>
              <p className="text-gray-500 text-sm mt-3">Analyzing image with AI...</p>
            </div>
          )}
        </div>
      </div>
